...
dojo.extend(dojo.dnd.Manager,{OFFSET_X:-10,OFFSET_Y:5,overSource:function(source){if(this.avatar){this.target=(source&&source.targetState!="Disabled")?source:null;
this.avatar.update()
...
if(body){if(dojo.isIE&&dojo.isIE<11){body.attachEvent("onactivate",function(evt){if(evt.srcElement.tagName.toLowerCase()!="body"){dijit._onFocusNode(evt.srcElement)
}});
...
A.form.submit()
}function dw(B){var A=dojo.i18n.getLocalization("aipo","locale");
var C=dojo.string.substitute(A.DW_STR,{dw_del:A.DW_DEL,dw_this:A.DW_THIS,dw_name:B.form.name});
if(confirm(C)){disableButton(B.form);
B.form.action=B.form.action+"?"+B.name+"=1";
B.form.submit()
}}function ews(A){disableButton(A.form);
A.form.action=A.form.action+"?"+A.name+"=1";
A.form.submit()
}function dws(B){var A=dojo.i18n.getLocalization("aipo","locale");
var C=dojo.string.substitute(A.DWS_STR,{dws_del:A.DWS_DEL,dws_sel:A.DWS_SEL,dws_name:B.form._name.value});
if(confirm(C)){disableButton(B.form);
B.form.action=B.form.action+"?"+B.name+"=1";
B.form.submit()
...
}function verifyCheckBox(F,G,E){var D=0;
var C;
for(C=0;
C<F.elements.length;
C++){if(F.elements[C].checked){D++
}}if(D==0){var B=dojo.i18n.getLocalization("aipo","locale");
var A=dojo.string.substitute(B.VERIFYCB_STR,{verifycb_sel:B.VERIFYCB_SEL,verifycb_gt_one:B.VERIFYCB_GT_ONE,verifycb_cb:B.VERIFYCB_CB});
alert(A);
return false
}else{return G(E)
...
aimluck.io.submit_callbackparams=function(C,A,B,H){aimluck.io.disableForm(C,true);
var G=dojo.byId(A+B);
if(G){dojo.style(G,"display","")
}var F=function(E){var I="";
if(dojo.isArray(E)&&E.length>0){if(E[0]=="PermissionError"){I+="<ul>";
I+="<li><span class='caution'>"+E[1]+"</span></li>";
I+="</ul>"
}else{I+="<ul>";
dojo.forEach(E,function(J){I+="<li><span class='caution'>"+J+"</span></li>"
});
I+="</ul>"
}}return I
};
try{dojo.xhrPost({url:C.action,timeout:30000,form:C,encoding:"utf-8",handleAs:"json-comment-filtered",headers:{X_REQUESTED_WITH:"XMLHttpRequest"},load:function(I,E){var K=[];
var J="";
if(I.err){J=F(I.err)
}K.push(J);
if(dojo.isArray(I.params)){dojo.forEach(I.params,function(L){K.push(L)
})
}H.apply(H,K);
G=dojo.byId(A+B);
if(G){dojo.style(G,"display","none")
}aimluck.io.disableForm(C,false)
},error:function(E){}})
}catch(D){}return false
};
...
aimluck.io.disableForm=function(C,D){function J(){return(((1+Math.random())*65536)|0).toString(16).substring(1)
}function H(){return(J()+J()+"-"+J()+"-"+J()+"-"+J()+"-"+J()+J()+J())
}if(C==null){return 
}if(D){var A=C.elements;
for(var F=0;
F<A.length;
F++){if((A[F].type=="submit"||A[F].type=="button")&&A[F].style.display!="none"){var B=H();
var I=document.createElement("span");
I.id=B;
I.className=A[F].className;
I.style.display=A[F].style.display;
A[F].parentNode.insertBefore(I,A[F]);
I.appendChild(document.createTextNode(A[F].value));
dojo.addClass(I,"auiButtonDisabled");
A[F].style.display="none";
dojo.addClass(A[F],B)
}}}else{var G=dojo.query(".auiButtonDisabled",C);
for(var F=0;
F<G.length;
F++){var B=G[F].id;
var E=dojo.query("."+B)[0];
dojo.removeClass(E,B);
E.style.display=G[F].style.display;
G[F].parentNode.removeChild(G[F])
}}};
...
aimluck.io.deleteSubmit=function(B){var A=dojo.i18n.getLocalization("aipo","locale");
var C=dojo.string.substitute(A.DW_STR,{dw_del:A.DW_DEL,dw_this:A.DW_THIS,dw_name:B.form._name.value});
if(confirm(C)){aimluck.io.disableForm(B.form,true);
aimluck.io.setHiddenValue(B);
B.form.action=B.form.action+"?"+B.name+"=1";
B.form.submit()
}};
aimluck.io.ajaxDeleteSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.DW_STR,{dw_del:A.DW_DEL,dw_this:A.DW_THIS,dw_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
}};
aimluck.io.ajaxEnableSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.ENABLESUBMIT_STR,{enableSubmit_this:A.ENABLESUBMIT_THIS,enableSubmit_enable:A.ENABLESUBMIT_ENABLE,enableSubmit_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
}};
aimluck.io.ajaxDisableSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.DISABLESUBMIT_STR,{disableSubmit_this:A.DISABLESUBMIT_THIS,disableSubmit_disable:A.DISABLESUBMIT_DISABLE,disableSubmit_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
}};
aimluck.io.deleteSubmitReturn=function(B,C){var A=dojo.i18n.getLocalization("aipo","locale");
var D=dojo.string.substitute(A.DW_STR,{dw_del:A.DW_DEL,dw_this:A.DW_THIS,dw_name:B.form._name.value});
if(confirm(D)){aimluck.io.disableForm(B.form,true);
aimluck.io.setHiddenValue(B);
B.form.action=B.form.action+"?"+B.name+"=1&action="+C;
B.form.submit()
}};
aimluck.io.multiDeleteSubmit=function(B){var A=dojo.i18n.getLocalization("aipo","locale");
var C=dojo.string.substitute(A.DWS_STR,{dws_del:A.DWS_DEL,dws_sel:A.DWS_SEL,dws_name:B.form._name.value});
if(confirm(C)){aimluck.io.disableForm(B.form,true);
aimluck.io.setHiddenValue(B);
B.form.action=B.form.action+"?"+B.name+"=1";
...
aimluck.io.ajaxMultiDeleteSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.DWS_STR,{dws_del:A.DWS_DEL,dws_sel:A.DWS_SEL,dws_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
}};
aimluck.io.ajaxMultiEnableSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.MULTIENABLESUBMIT_STR,{multiEnableSubmit_sel:A.MULTIENABLESUBMIT_SEL,multiEnableSubmit_enable:A.MULTIENABLESUBMIT_ENABLE,multiEnableSubmit_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
}};
aimluck.io.ajaxMultiDisableSubmit=function(C,B,D,E,G){var A=dojo.i18n.getLocalization("aipo","locale");
var F=dojo.string.substitute(A.MULTIDISABLESUBMIT_STR,{multiDisableSubmit_sel:A.MULTIDISABLESUBMIT_SEL,multiDisableSubmit_disable:A.MULTIDISABLESUBMIT_DISABLE,multiDisableSubmit_name:C.form._name.value});
if(confirm(F)){aimluck.io.disableForm(C.form,true);
aimluck.io.setHiddenValue(C);
C.form.action=B;
aimluck.io.submit(C.form,D,E,G)
...
aimluck.io.verifyCheckbox=function(F,G,E){var D=0;
var C;
for(C=0;
C<F.elements.length;
C++){if(F.elements[C].checked){D++
...
}}if(C==0){var J=dojo.i18n.getLocalization("aipo","locale");
var K=dojo.string.substitute(J.VERIFYCB_STR,{verifycb_sel:J.VERIFYCB_SEL,verifycb_gt_one:J.VERIFYCB_GT_ONE,verifycb_cb:J.VERIFYCB_CB});
alert(K);
return false
...
aimluck.io.replaceToAutoCRString=function(G){var D="";
var F=4;
var C=G.length;
...
B<E;
B++){var A=B*F;
D=D.concat(G.substr(A,F)).concat("<wbr/>")
}if(E*F<C){D=D.concat(G.substr(E*F))
}return D
};
aimluck.io.addFileToList=function(D,E,B){var C=dojo.i18n.getLocalization("aipo","locale");
if(D.parentNode.style.display=="none"){D.parentNode.style.display=""
}if(document.all){var A=document.createElement("li");
A.setAttribute("data-fileid",E);
A.setAttribute("data-filename",B);
A.innerHTML="<span>"+aimluck.io.replaceToAutoCRString(B)+'</span><span class="deletebutton" onclick="aimluck.io.removeFileFromList(this.parentNode.parentNode,this.parentNode);">'+C.DELETE_STR+"</span>";
...
aimluck.io.replaceFileToList=function(D,E,B){var C=dojo.i18n.getLocalization("aipo","locale");
if(document.all){var A=document.createElement("li");
A.setAttribute("data-fileid",E);
A.setAttribute("data-filename",B);
A.innerHTML="<span>"+aimluck.io.replaceToAutoCRString(B)+'</span><span class="deletebutton" onclick="aimluck.io.removeFileFromList(this.parentNode.parentNode,this.parentNode);">'+C.DELETE_STR+"</span>";
...
aimluck.io.removeFileFromList=function(B,A){B.removeChild(A);
var C=document.getElementById("modalDialog");
if(C){var D=document.getElementById("wrapper");
D.style.minHeight=C.clientHeight+"px"
}};
...
aimluck.io.onTextFieldFocus=function(){var A=document.getElementById("mobileHeader_v3");
if(A){if(!aipo.userAgent.isAndroid()){A.style.position="absolute";
A.style.top="0px"
}}};
aimluck.io.onTextFieldBlur=function(){var A=document.getElementById("mobileHeader_v3");
if(A){if(!aipo.userAgent.isAndroid()){A.style.position="";
A.style.top="0px"
}}};
...
dojo.query("a.customizeMenuIcon,a.menubarOpenButton").forEach(function(A){dojo.connect(A,"onmouseenter",null,function(){dojo.addClass(this,"customizeMenuIconMouseenter")
});
dojo.connect(A,"onmouseleave",null,function(){dojo.removeClass(this,"customizeMenuIconMouseenter")
})
});
bodyHandle=dojo.connect(dojo.query("body")[0],"onclick",null,function(){if(dojo.query("a.customizeMenuIconMouseenter").length==0){dojo.query("div.menubar").style("display","none")
}});
if(aipo.onloadSmartPhone!=null){aipo.onloadSmartPhone()
}};
...
},isAndroid2:function(){var A=this.androidVersion();
return !!A&&A[1]==2
},isAndroid4:function(){var A=this.androidVersion();
return !!A&&A[1]==4
},isAndroid4_0:function(){var A=this.androidVersion();
return !!A&&A[1]==4&&A[2]==0
},androidVersion:function(){return this.__userAgent.match(/android ([\d]+)\.([\d]+)\.([\d]+)/)
...
aipo.arrayContains=function(A,C){for(var B=0;
B<A.length;
...
A.requireLocalization("aipo","locale");
...
},error:function(G,J,I,F){var E=A.i18n.getLocalization("aipo","locale");
var H=A.string.substitute(E.XHRERROR_STR,{xhrError_error:E.XHRERROR_ERROR,xhrError_loading:E.XHRERROR_LOADING,xhrError_failed:E.XHRERROR_FAILED});
A.byId("content-"+C).innerHTML=H;
A.html.setVisibility(A.byId("content-"+C),true);
A.html.setDisplay(A.byId("indicator-"+C),false)
},timeout:function(G,J,I,F){var E=A.i18n.getLocalization("aipo","locale");
var H=A.string.substitute(E.XHRTIMEOUT_STR,{xhrTimeout_error:E.XHRTIMEOUT_ERROR,xhrTimeout_timeout:E.XHRTIMEOUT_TIMEOUT});
A.byId("content-"+C).innerHTML=H;
A.html.setVisibility(A.byId("content-"+C),true);
A.html.setDisplay(A.byId("indicator-"+C),false)
},timeoutSeconds:10})
...
B.requireLocalization("aipo","locale");
var A=B.i18n.getLocalization("aipo","locale");
B.declare("aimluck.widget.Contentpane",[dijit.layout.ContentPane],{loadingMessage:"<div class='indicator'>"+A.LOADING_STR+"</div>",errorMessage:"",extractContent:false,parseOnLoad:true,refreshOnShow:true,params:new Array(),reloadIds:new Array(),viewPage:function(C){this.href=C;
return this._prepareLoad(true)
},setParam:function(C,D){this.params[C]=D
},setReloadIds:function(C){this.reloadIds=C
...
if(B.isObject(this.ioArgs)){B.mixin(E,this.ioArgs)
}var C=this._xhrDfd=(this.ioMethod||B.xhrPost)(E);
C.addCallback(function(F){D.clearParams();
D.clearReloadIds();
try{D.onDownloadEnd.call(D);
D._isDownloaded=true;
D.setContent.call(D,F)
}catch(G){D._onError.call(D,"Content",G)
}delete D._xhrDfd;
return F
});
C.addErrback(function(F){if(!C.cancelled){D._onError.call(D,"Download",F)
}delete D._xhrDfd;
return F
})
}})
...
dojo.declare("aimluck.widget.DialogSub",[aimluck.widget.Dialog,dijit.Dialog],{templateString:"<div id='modalDialog' class='modalDialog' dojoattachpoint='wrapper'><span dojoattachpoint='tabStartOuter' dojoonfocus='trapTabs' dojoonblur='clearTrap'tabindex='0'></span><span dojoattachpoint='tabStart' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span><div dojoattachpoint='containerNode' style='position: relative; z-index: 2;'></div><span dojoattachpoint='tabEnd' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span><span dojoattachpoint='tabEndOuter' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span></div>"});
dojo.declare("aimluck.widget.Dialog",[dijit.Dialog],{loadingMessage:"<div class='auiPopup indicatorDialog center'><i class='auiIcon auiIconIndicator'></i> "+nlsStrings.LOADING_STR+"</div>",errorMessage:"<div class='auiPopup indicatorDialog center'>${errorState}</div>",templateString:null,templateString:"<div id='modalDialog' class='modalDialog' dojoattachpoint='wrapper'><span dojoattachpoint='tabStartOuter' dojoonfocus='trapTabs' dojoonblur='clearTrap'tabindex='0'></span><span dojoattachpoint='tabStart' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span><div dojoattachpoint='containerNode' style='position: relative; z-index: 2;'></div><span dojoattachpoint='tabEnd' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span><span dojoattachpoint='tabEndOuter' dojoonfocus='trapTabs' dojoonblur='clearTrap' tabindex='0'></span></div>",duration:10,extractContent:false,parseOnLoad:true,refreshOnShow:true,isPositionLock:false,params:new Array(),reloadIds:new Array(),_portlet_id:null,_callback:null,eventList:[],_setup:function(){this._modalconnects=[];
if(this.titleBar){this._moveable=new dojo.dnd.Moveable(this.domNode,{handle:this.titleBar});
var A=this.domNode;
...
if(L.indexOf("iphone")>-1||L.indexOf("android")>-1){if(!!document.documentElement.scrollTop){document.documentElement.scrollTop=0
}else{if(!!document.body.scrollTop){document.body.scrollTop=0
}}}if(aipo.userAgent.isAndroid2()){var A=dojo.byId("wrapper");
dojo.style(A,"display","none");
var K=dojo.byId("modalDialog");
dojo.style(K,"top","0px");
dojo.style(K,"margin","0")
}if(aipo.userAgent.isAndroid()){var G=this;
var H=function(M){M.preventDefault();
return false
};
var C=dojo.query("input,select,button,a",this.domNode);
var D=dojo.byId("appsNavigation_v2");
var E=dojo.query("input,select,button,a",D);
var J=dojo.byId("auiWidgetsArea");
var I=dojo.query("input,select,button,a",J);
var F=C;
F=F.concat(E);
F=F.concat(I);
dojo.query("input,select,button,a").forEach(function(M){if(!aipo.arrayContains(F,M)){G.eventList.push(dojo.connect(M,"click",H))
}});
dojo.query("input,select,button").forEach(function(N,M){if(!aipo.arrayContains(F,N)&&!N.disabled){dojo.addClass(N,"disabled-by-dialog");
N.disabled=true
}})
}var B=dojo.byId(this.widgetId);
if(B){B.focus();
if(this._callback!=null){this._callback.call(this._callback,this._portlet_id)
}}dojo.query("#modalDialog input, #modalDialog textarea").forEach(function(M){dojo.connect(M,"onfocus",aimluck.io.onTextFieldFocus);
dojo.connect(M,"onblur",aimluck.io.onTextFieldBlur)
});
dojo.query("#modalDialog form").forEach(function(M){dojo.connect(M,"onsubmit",aimluck.io.onTextFieldBlur)
})
...
if(this.id=="imageDialog"){if(B.width==0){B.marginLeft="-150px"
}else{B.marginLeft=Math.floor(-B.width.replace("px","")/2)+"px"
...
},hide:function(){var B=document.getElementById("wrapper");
...
}if(aipo.userAgent.isAndroid()){for(var A=0;
A<this.eventList.length;
A++){dojo.disconnect(this.eventList[A])
}this.eventList=[];
dojo.query("input.disabled-by-dialog,select.disabled-by-dialog,button.disabled-by-dialog").forEach(function(D,C){dojo.removeClass(D,"disabled-by-dialog");
D.disabled=false
})
}if(aipo.userAgent.isAndroid2()){var B=dojo.byId("wrapper");
...
},addOption:function(C,D,E,F){aimluck.io.addOption(C,D,E,F)
},addOptionSync:function(G,H,I){var D=B.byId(this.memberToId);
var C=B.byId(this.selectId);
if(this.memberLimit!=0&&D.options.length>=this.memberLimit){return 
}if(document.all){var F=document.createElement("OPTION");
F.value=G;
F.text=H;
F.selected=I;
...
E.text=H;
E.selected=I;
if(D.options.length==1&&D.options[0].value==""){D.options.remove(0);
C.options.remove(0)
}D.add(F,D.options.length);
C.add(E,C.options.length)
}else{var F=document.createElement("OPTION");
F.value=G;
F.text=H;
F.selected=I;
var E=document.createElement("OPTION");
...
C.removeChild(D.options[0])
}D.insertBefore(F,D.options[D.options.length]);
...
for(j=0;
j<H.length;
j++){if(H[j].value==C[i].value){F=true;
break
}}if(F){continue
}var E=document.createElement("OPTION");
E.value=C[i].value;
...
i++){if(D[i].selected){C.removeChild(D[i]);
i-=1
}}}},removeMemberSync:function(){var E=B.byId(this.memberToId);
var D=B.byId(this.selectId);
if(document.all){var F=E.options;
var C=D.options;
for(i=0;
i<F.length;
...
C.remove(i);
i-=1
}}}else{var F=E.options;
var C=D.options;
for(i=0;
i<F.length;
i++){if(F[i].selected){E.removeChild(F[i]);
D.removeChild(C[i]);
i-=1
}}}},inputMemberSync:function(){var C=B.byId(this.selectId);
var D=B.byId(this.inputId);
var E="";
var F=C.options;
for(i=0;
i<F.length;
...
},changeGroup:function(C){var F=C.target.options[C.target.selectedIndex].value;
var D=this.changeGroupUrl+"&groupname="+F;
var E={url:D,key:this.memberFromOptionKey,value:this.memberFromOptionValue};
aimluck.io.createOptions(this.memberFromId,E)
},onMemberAddClick:function(C){this.addMember(B.byId(this.memberFromId),B.byId(this.memberToId));
this.addMember(B.byId(this.memberFromId),B.byId(this.selectId));
this.inputMemberSync()
},onMemberRemoveClick:function(C){this.removeMemberSync();
this.inputMemberSync()
}})
}}});
...
},addOptionSync:function(E,F,G){var C=B.byId(this.memberToId);
...
i++){if(D[i].selected){C.removeChild(D[i]);
i-=1
}}}},removeMemberSync:function(){var C=B.byId(this.memberToId);
...
this.setWrapperHeight()
},setWrapperHeight:function(){var C=document.getElementById("modalDialog");
if(C){var D=document.getElementById("wrapper");
D.style.minHeight=C.clientHeight+"px"
}}})
}}});
...
var J=B[E.getDay()];
var L=A.i18n.getLocalization("aipo","locale");
var M=A.string.substitute(L.DATE_FORMAT,{year:K,month:D,date:C,day:J});
var G=A.byId(this.dateId+"_view");
G.innerHTML=M;
var N=A.byId(this.dateId);
N.value=K+"/"+D+"/"+C;
...
var K=A.i18n.getLocalization("aipo","locale");
F.innerHTML=K.DISABLED_DATE;
var I=A.byId(this.dateId+"_year");
I.value="";
...
}}else{var M=A.byId(this.dateId);
if((!M.value)||(M.value=="")){this.setValue(new Date())
}else{var L=M.value.split("/");
if(L.length==3){var J=L[0];
var C=L[1]-1;
var H=L[2];
...
A.declare("aipo.widget.DropdownActivityChecker",[aimluck.widget.Dropdown],{initValue:"",displayCheck:"",iconURL:"",iconAlt:"",iconWidth:"",iconHeight:"",extendClass:"",eventList:[],callback:function(){},templateString:'<div class="dijit dijitLeft dijitInline"\n\tdojoAttachEvent="onmouseenter:_onMouse,onmouseleave:_onMouse,onmousedown:_onMouse,onclick:_onDropDownClick,onkeydown:_onDropDownKeydown,onblur:_onDropDownBlur,onkeypress:_onKey"\n\t><div class="" type="${type}"\n\t\tdojoAttachPoint="focusNode,titleNode" waiRole="button" waiState="haspopup-true,labelledby-${id}_label"\n\t\t><div class="" \tdojoAttachPoint="containerNode,popupStateNode"\n\t\tid="${id}_label"><div id="activitychecker" class="zero activitycheckerstyle ${extendClass}"></div><span class="mb_hide">お知らせ</span><span class="pc_hide"><i class="icon-bell-alt"></i></span></div></div></div>\n',postCreate:function(){this.inherited(arguments);
this.dropDown=new aipo.widget.ActivityList({},"activityLiteList")
},_openDropDown:function(){this.inherited(arguments);
if(A.byId("activitycheckerContainer")){A.addClass(A.byId("activitycheckerContainer"),"active")
}this.dropDown.reload();
var J=window.navigator.userAgent.toLowerCase();
if(J.indexOf("iphone")>-1||J.indexOf("android")>-1){if(!!document.documentElement.scrollTop){document.documentElement.scrollTop=0
}else{if(!!document.body.scrollTop){document.body.scrollTop=0
}}}if(aipo.userAgent.isAndroid()){var F=this;
var G=function(K){K.preventDefault();
return false
};
var B=A.query("input,select,button,a",this.domNode);
var C=A.byId("appsNavigation_v2");
var D=A.query("input,select,button,a",C);
var H=A.byId("auiWidgetsArea");
var I=A.query("input,select,button,a",H);
var E=B;
E=E.concat(D);
E=E.concat(I);
A.query("input,select,button,a").forEach(function(K){if(!aipo.arrayContains(E,K)){F.eventList.push(A.connect(K,"click",G))
}});
A.query("input,select,button").forEach(function(L,K){if(!aipo.arrayContains(E,L)&&!L.disabled){A.addClass(L,"disabled-by-activity");
L.disabled=true
}})
}},_closeDropDown:function(){this.inherited(arguments);
if(A.byId("activitycheckerContainer")){A.removeClass(A.byId("activitycheckerContainer"),"active")
}if(document.getElementById("wrapper")){document.getElementById("wrapper").style.minHeight=""
}if(aipo.userAgent.isAndroid()){for(var B=0;
B<this.eventList.length;
B++){A.disconnect(this.eventList[B])
}this.eventList=[];
A.query("input.disabled-by-activity,select.disabled-by-activity,button.disabled-by-activity").forEach(function(D,C){A.removeClass(D,"disabled-by-activity");
D.disabled=false
})
...
A.removeClass("activitychecker","zero")
}}A.addClass("activitychecker","counter")
...
A.provide("aipo.fileupload.widget.YoutubeDialog");
A.provide("aipo.fileupload.widget.YoutubeDialogUnderlay");
...
A.declare("aipo.fileupload.widget.YoutubeDialog",[aimluck.widget.Dialog],{loadingMessage:"<div class='indicator'>読み込み中...</div>",templateCssString:"youtubeDialog",templateString:"<div id='youtubeDialog' class='auiPopup imgPopup ${templateCssString}' dojoattachpoint='wrapper' style='max-width:640px;left:50%;top:50%;margin:-200px 0 0 -320px;'></div>",_setup:function(){this._modalconnects=[];
if(this.titleBar){this._moveable=new A.dnd.Moveable(this.domNode,{handle:this.titleBar})
...
A.require("aimluck.widget.Dialog");
A.declare("aipo.fileupload.widget.FileuploadViewDialogUnderlay",[aimluck.widget.DialogUnderlay],{templateString:"<div class='fileuploadViewDialogUnderlayWrapper modalDialogUnderlayWrapper' id='${id}_underlay'><div class='fileuploadViewDialogUnderlay modalDialogUnderlay' dojoAttachPoint='node'></div></div>"});
...
var L=parseInt(I)||0;
if(dojo.byId("messagechecker")!=undefined){L+=parseInt(dojo.byId("messagechecker").innerHTML)
}if(dojo.byId("supportchecker")!=undefined){L+=parseInt(dojo.byId("supportchecker").innerHTML)
